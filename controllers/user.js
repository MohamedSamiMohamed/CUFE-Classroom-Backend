...
exports.getMe = async (req, res) => {
    if (req.user.role === "instructor") {
        ...
        return res.send(_.pick(instructor, ['_id', 'userName', 'email', 'firstName', 'lastName', 'type', 'birthDate', 'courses']))
    }
        ...
        return res.send(_.pick(admin, ['_id', 'userName', 'email', 'firstName', 'lastName', 'type', 'birthDate', 'courses']))
    }
    else {
        let user = await User.findById(req.user._id)
        if (!user) return res.status(404).send("sorry, something wrong has been occured")
        return res.send(_.pick(user, ['_id', 'userName', 'email', 'firstName', 'lastName', 'type', 'birthDate']))
    }
}
...
exports.getUser = async (req, res) => {
    ...
    if (user.type === "instructor" || user.type == "admin") {
        ...
        return res.send(_.pick(user, ['_id', 'userName', 'email', 'firstName', 'lastName', 'type', 'birthDate', 'courses']))
    }
    else {
        return res.send(_.pick(user, ['_id', 'userName', 'email', 'firstName', 'lastName', 'type', 'birthDate']))
    }
...
exports.getUserByEmail = async (req, res) => {
    ...
    if (user.type === "instructor" || user.type == "admin") {
        user = await user.populate('courses', '-instructor -qa -weeks')
        return res.status(200).send(_.pick(user, ['_id', 'userName', 'email', 'firstName', 'lastName', 'type', 'birthDate', 'courses']))
    }
    else {
        return res.status(200).send(_.pick(user, ['_id', 'userName', 'email', 'firstName', 'lastName', 'type', 'birthDate']))
    }
}
